...
class ManageRasters():
    ...
    def normalize(self, bands=[1,2,3]):
        '''Performs bandwise normalization on images'''
        for file in self.rasters:
    ...
    def TOA(self, target_file=None):
        '''Calculates top of the atmosphere pixel values for .tif files. Requires
        the use of self.get_band_meta() beforehand.
        
        Parameters
        ----------
        target_file: str (optional)
            String specifying a pattern in the filename for the desired file(s)
    ...
    def threshold_ndvi(self, min_value=-1, max_value=1):
        ...
        Typically should not be necessary unless there exists an issue with
        image pixel values.
        '''
    ...
    def find_dataset_mean(self):
        '''Finds the mean of each image channel across an entire dataset'''
        ch1_mean = []
        ch2_mean = []
        ch3_mean= []
